```bash
       ...
       configure_dns() {
           ...
           # Read current resolv.conf, exclude Shecan DNS entries, comment out other nameservers, keep other lines
           {
               while IFS= read -r line; do
                   if [[ $line =~ ^nameserver\ (185.51.200.2|178.22.122.100) ]] || [[ $line =~ ^#nameserver\ (185.51.200.2|178.22.122.100) ]]; then
                       continue
                   elif [[ $line =~ ^nameserver ]]; then
                       echo "#$line"
                   else
                       echo "$line"
                   fi
